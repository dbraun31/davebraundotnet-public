...
permutation_cluster_test <- function(item, low_anchor, 
                               time_window_min=0.25, bads = c(10), initial_alpha=.01) {
    # --- PARAMETERS --- #
    # ------------------ #
    # item (str): Name of experience sampling item
    # low_anchor (str): Name of the low anchor on the scale of item
    ...
    # Prepare path
    data_root <- path('analysis', 'data', 'derivatives', 'hep', '06-evoked-clean')
    
    # --- IMPORT DATA --- #
    # (it already has median split from behavioral data)
    # DF 
    # ROWS: each obs is average across epochs for a single probe at a single time 
        # point per subject. 
    # COLS: subject, condition, probe, epochs_per_probe, time, ...electrodes...
    eeg <- read_feather(here(path(data_root, paste0('hep_', item, '.feather'))))
    eeg$subject <- as.numeric(eeg$subject)
    
    # --- FORMAT DATA --- #
    # Find bad subjects
    low_anchor <- tolower(low_anchor)
    bads <- unique(c(bads, drop_condition_imbalance(eeg, low_anchor)))
    
    # --- RUN STATISTICS --- #  
    result <- py$permutation_cluster_test(item=item, low_anchor=low_anchor, bads=bads, 
                                          time_window_min=time_window_min, initial_alpha=initial_alpha)
